...
def delivery_date(start, description):
    ...
    # due two hours after start
    if description == 'NOW':  
        due_dt = start_dt + timedelta(hours=2)
        
    # due 5pm if before 1pm, else 1pm next day
    ...
    # due 5pm Friday if Mon-Wed, else 8pm Sunday
    elif description == 'EOW':  
        wkday_num = start_d.weekday()
        if wkday_num <= 2:
            due_dt = datetime.combine(
                start_d + timedelta(days=4 - wkday_num), 
                time(hour=17)
            )
        else:
            due_dt = datetime.combine(
                start_d + timedelta(days=6 - wkday_num), 
                time(hour=20)
            )

    # due 8am on first workday of target month
    elif match := re.fullmatch(r'(\d+)M', description):  
    ...
    # due 8am on last workday of target quarter
    elif match := re.fullmatch(r'Q(\d+)', description):  
